...
import { map, toArray } from "../../src/operator";
...
const defaultSpecCase = (observable, project, publisher, withUnsubscribe, expectation, done) => {
    ...
    const mapped = observable.pipe(map(project, (withUnsubscribe ? () => (unsubscribed = true) : false)), toArray());
...
describe("fre operator/map functionality", function () {

    // set-up spec testing feature-set
    it("should carry-out the map operator against an Observable", function (done) {
        defaultSpecCase(new Observable(helpers.withComplete), (v) => "test" + v, undefined, true, "[\"test1\",\"test2\",\"test3\",\"test4\"]", done);
    });

    // set-up spec testing feature-set
    it("should carry-out the map operator against a Subject", function (done) {
        defaultSpecCase(new Subject(), (v) => "test" + v, helpers.withComplete, true, "[\"test1\",\"test2\",\"test3\",\"test4\"]", done);
    });
    
    // set-up spec testing feature-set
    it("should carry-out the map operator against a BehaviourSubject", function (done) {
        defaultSpecCase(new BehaviourSubject(),(v) => "test" + v, helpers.withComplete, false, "[\"testundefined\",\"test1\",\"test2\",\"test3\",\"test4\"]", done);
    });
