...
import { last, toArray } from "../../src/operator";
...
const defaultSpecCase = (observable, project, publisher, withUnsubscribe, expectation, done) => {
    ...
    const lastMessage = observable.pipe(last(project, (withUnsubscribe ? () => (unsubscribed = true) : false)), toArray());
...
describe("fre operator/last functionality", function () {

    // set-up spec testing feature-set
    it("should carry-out the last operator against an Observable", function (done) {
        defaultSpecCase(new Observable(helpers.withComplete), (v) => v > 2, undefined, true, "[4]", done);
    });

    // set-up spec testing feature-set
    it("should carry-out the last operator against a Subject", function (done) {
        defaultSpecCase(new Subject(), (v) => v > 2, helpers.withComplete, true, "[4]", done);
    });
    
    // set-up spec testing feature-set
    it("should carry-out the last operator against a BehaviourSubject", function (done) {
        defaultSpecCase(new BehaviourSubject(), (v) => v > 2, helpers.withComplete, false, "[4]", done);
    });
