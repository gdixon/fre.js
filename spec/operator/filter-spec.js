...
import { filter, toArray } from "../../src/operator";
...
const defaultSpecCase = (observable, project, publisher, withUnsubscribe, expectation, done) => {
    ...
    const filtered = observable.pipe(filter(project, (withUnsubscribe ? () => (unsubscribed = true) : false)), toArray());
    // place a subscription carry out the work
    filtered.subscribe((message) => {
...
describe("fre operator/filter functionality", function () {


    // set-up spec testing feature-set
    it("should carry-out the filter operator against an Observable", function (done) {
        defaultSpecCase(new Observable(helpers.withComplete), (v) => v > 2, undefined, true, "[3,4]", done);
    });

    // set-up spec testing feature-set
    it("should carry-out the filter operator against a Subject", function (done) {
        defaultSpecCase(new Subject(), (v) => v > 2, helpers.withComplete, true, "[3,4]", done);
    });
    
    // set-up spec testing feature-set
    it("should carry-out the filter operator against a BehaviourSubject", function (done) {
        defaultSpecCase(new BehaviourSubject(), (v) => v > 2, helpers.withComplete, false, "[3,4]", done);
    });
